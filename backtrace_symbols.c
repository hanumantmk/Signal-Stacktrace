...
static __thread void load_funcs(void)
{
...
static __thread asymbol **syms;        /* Symbol table.  */
...
static __thread bfd_vma pc;
static __thread const char *filename;
static __thread const char *functionname;
static __thread unsigned int line;
static __thread int found;
...
char * backtrace_symbols_str(char * prefix)
{
  ...
  int nframes = backtrace(backtrace_buffer, BACKTRACE_BUFFER_SIZE);
  char ** string_symbols = backtrace_symbols(backtrace_buffer, nframes);
...
{
  char * out = backtrace_symbols_str(prefix);
